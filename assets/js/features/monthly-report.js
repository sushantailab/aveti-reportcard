...
async function monthlyBuildData(selected,students){
  ...
  return {...base,month:monthlyMonthLabel(latestKey),summary:overall==null?'No scored assessments are available for this student in the selected period.':`${selected.name} has an overall average of ${overall}% across ${currentScores.length} attended assessment${currentScores.length===1?'':'s'}. ${best?`${best[0]} is the strongest subject.`:''} ${weak&&weak!==best?`${weak[0]} needs additional practice.`:''}`,kpis:[['Overall Average',overall==null?'—':`${overall}%`,''],['Tests Conducted',String(periodTests.length),''],['Improvement',improvement==null?'—':`${improvement>=0?'+':''}${improvement}%`,''],['Exam Attendance',attendance==null?'—':`${attendance}%`,''],['Focus Subject',weak?weak[0]:'—',''],['Discipline','Not available','']],subjects:subjectRows,trend:{labels:subjectRows.map(s=>monthlyShortSubject(s[0])),student:subjectRows.map(s=>parseFloat(s[2])||0),klass:subjectRows.map(s=>parseFloat(s[3])||0),topper:subjectRows.map(s=>parseFloat(s[4])||0)},highlights:[best&&['Best performance',best[0],'Highest score this month',best[2],'best'],weak&&['Needs revision',weak[0],'Focus subject',weak[2],'needs'],improvement!=null&&[improvement>=0?'Most improved':'Declined this month',selected.name,'Compared with previous month',`${improvement>=0?'+':''}${improvement}%`,improvement>=0?'improved':'needs']].filter(Boolean),insight:[best?`${best[0]} is the strongest subject this month.`:'No strength identified yet.',weak?`${weak[0]} needs more practice.`:'Add more assessments to identify a focus area.',improvement!=null?`Performance changed ${improvement>=0?'up':'down'} ${Math.abs(improvement)} points from the previous month.`:'Keep attempting regular chapter tests.'],plan:[weak?`Revise ${weak[0]} concepts`:'Review the next available chapter','Practise weekly worksheets','Discuss progress with the teacher','Attempt more practice tests']};
}
...
function monthlySubjectHealth(subjects){
  return subjects.map(s=>{
    const score=parseFloat(String(s[2]).replace('%',''))||0;
    const status=score>=80?'Strong':score>=70?'Good':score>=60?'Average':'Needs practice';
    const tone=score>=80?'strong':score>=70?'good':score>=60?'average':'needs';
    return `<article class="health-${tone}"><span class="health-icon">${tone==='strong'?'★':tone==='good'?'✓':tone==='average'?'◆':'!'}</span><b>${escMonthly(s[0])}</b><small>${status}</small></article>`;
  }).join('');
}
...
async function monthlyReport(){
  ...
  document.getElementById('app').innerHTML=`<main class="monthly-report" id="monthlyReport">
    ...
    <section class="monthly-summary"><h2>Executive summary</h2><p>${escMonthly(d.summary)}</p></section>
    ...
    <section class="monthly-panel monthly-insight-plan"><div><h2>Teacher insight</h2><div class="monthly-insight-cards"><article class="insight-strength"><b>Strength</b><span>${escMonthly(d.insight[0]||'Not available')}</span></article><article class="insight-focus"><b>Focus</b><span>${escMonthly(d.insight[1]||'Not available')}</span></article><article class="insight-recommendation"><b>Recommendation</b><span>${escMonthly(d.insight[2]||'Not available')}</span></article></div></div><div><h2>Next month plan</h2><div class="monthly-plan-grid">${d.plan.map((x,i)=>`<div><span>${i+1}</span><b>${x}</b></div>`).join('')}</div></div></section>
    <div class="monthly-page-break"></div>
    <section class="monthly-page-two"><h2>Executive performance summary</h2><div class="monthly-overall">${[['Overall progress',d.kpis[0][1],d.kpis[0][1]==='—'?'Not available':'Based on attended assessments'],['Improvement',d.kpis[2][1],d.kpis[2][1]==='—'?'Not available':(String(d.kpis[2][1]).startsWith('-')?'Declined from previous month':'Improved from previous month')],['Consistency',d.kpis[1][1]==='0'?'Not available':'Very good',d.kpis[1][1]==='0'?'No tests conducted':'Regular participation'],['Discipline',d.kpis[5][1],d.kpis[5][1]==='Not available'?'No discipline rating recorded':'Teacher-rated']].map((x,i)=>`<article class="${i===2?'best':'good'}"><span class="summary-icon">${monthlyIcon(['chart','trend','check','star'][i])}</span><small>${x[0]}</small><strong>${escMonthly(x[1])}</strong><em>${x[2]}</em></article>`).join('')}</div><h2>Achievement badges</h2><div class="monthly-badges">${['Science star','Most improved student','Perfect exam attendance','Homework champion'].map(x=>`<div><span>${monthlyIcon('badge')}</span><b>${x}</b></div>`).join('')}${d.kpis[5][1]!=='Not available'?`<div><span>${monthlyIcon('badge')}</span><b>Excellent discipline</b></div>`:''}</div><section class="monthly-parent-guide"><h2>Parent action guide</h2><ul><li>Encourage daily reading.</li><li>Ask one question after every class.</li><li>Help your child attempt more practice tests.</li></ul></section><div class="monthly-signatures"><div><b>Academic head</b><strong>AVETI Learning Tuition Center</strong><hr><span>Generated: ${new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(new Date())}</span></div></div></section>
  </main>`;
}
