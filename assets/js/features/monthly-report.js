...
async function monthlyBuildData(selected,students){
  ...
  const discipline='Not available';
  const attendedLabel=`${currentScores.length} of ${periodTests.length} assessment${periodTests.length===1?'':'s'} attended`;
  return {...base,month:monthlyMonthLabel(latestKey),summary:overall==null?'No scored assessments are available for this student in the selected period.':`${selected.name} has an overall average of ${overall}% (${attendedLabel}). ${best?`${best[0]} is the strongest subject.`:''} ${needs?`${needs[0]} needs additional practice.`:'No separate subject needs revision yet.'}`,kpis:[['Overall Average',overall==null?'—':`${overall}%`,''],['Tests Conducted',String(periodTests.length),''],['Improvement',improvement==null?'—':`${improvement>=0?'+':''}${improvement}%`,''],['Exam Attendance',attendance==null?'—':`${attendance}%`,`${studentAttended} of ${periodTests.length} exams attended`],['Focus Subject',needs?needs[0]:'No single focus',''],['Discipline',discipline,'']],subjects:subjectRows,trend:{labels:subjectRows.map(s=>monthlyShortSubject(s[0])),student:subjectRows.map(s=>parseFloat(s[2])||0),klass:subjectRows.map(s=>parseFloat(s[3])||0),topper:subjectRows.map(s=>parseFloat(s[4])||0)},highlights:[],insight:[best?`${best[0]} is the strongest subject this month.`:'No strength identified yet.',needs?`${needs[0]} needs more practice.`:'No subject is currently below the revision threshold.',improvement!=null?`Performance changed ${improvement>=0?'up':'down'} ${Math.abs(improvement)} points from the previous month.`:'Keep attempting regular chapter tests.'],plan:[needs?`Revise ${needs[0]} concepts`:'Review the next available chapter','Practise weekly worksheets','Discuss progress with the teacher','Attempt more practice tests']};
}
