...
const MONTHLY_SAMPLE = {
  ...
  kpis: [['Overall Average','82%',''],['Tests Conducted','18',''],['Improvement','+7%',''],['Exam Attendance','100%',''],['Focus Subject','English',''],['Discipline','Not available','']],
...
async function monthlyBuildData(selected,students){
  ...
  if(!selected) return {...base,subjects:[],trend:{labels:[],student:[],klass:[],topper:[]},month:'No test data',kpis:[['Overall Average','—',''],['Tests Conducted','0',''],['Improvement','—',''],['Exam Attendance','—',''],['Focus Subject','—',''],['Discipline','Not available','']],highlights:[],insight:['No tests are available for this student yet.','Add an Aveti test to begin monthly tracking.','Monthly insights will appear after marks are entered.','Continue attempting chapter tests.'],plan:['Enter the next assessment marks','Review the first available report','Schedule targeted revision','Attempt more practice tests']};
  ...
  return {...base,month:monthlyMonthLabel(latestKey),summary:overall==null?'No scored assessments are available for this student in the selected period.':`${selected.name} has an overall average of ${overall}%. ${best?`${best[0]} is the strongest subject.`:''} ${needs?`${needs[0]} needs additional practice.`:'No separate subject needs revision yet.'}`,kpis:[['Overall Average',overall==null?'—':`${overall}%`,''],['Tests Conducted',String(periodTests.length),''],['Improvement',improvement==null?'—':`${improvement>=0?'+':''}${improvement}%`,''],['Exam Attendance',attendance==null?'—':`${attendance}%`,`${studentAttended} of ${periodTests.length} exams attended`],['Focus Subject',needs?needs[0]:'No single focus',''],['Discipline','Not available','']],subjects:subjectRows,trend:{labels:subjectRows.map(s=>monthlyShortSubject(s[0])),student:subjectRows.map(s=>parseFloat(s[2])||0),klass:subjectRows.map(s=>parseFloat(s[3])||0),topper:subjectRows.map(s=>parseFloat(s[4])||0)},highlights:[],insight:[best?`${best[0]} is the strongest subject this month.`:'No strength identified yet.',needs?`${needs[0]} needs more practice.`:'No subject is currently below the revision threshold.',improvement!=null?`Performance changed ${improvement>=0?'up':'down'} ${Math.abs(improvement)} points from the previous month.`:'Keep attempting regular chapter tests.'],plan:[needs?`Revise ${needs[0]} concepts`:'Review the next available chapter','Practise weekly worksheets','Discuss progress with the teacher','Attempt more practice tests']};
}
