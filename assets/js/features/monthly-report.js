...
const MONTHLY_SAMPLE = {
  ...
  kpis: [['Overall Average','82%',''],['Tests Conducted','18',''],['Improvement','+7%',''],['Exam Attendance','100%',''],['Focus Subject','English','']],
...
async function monthlyBuildData(selected,students){
  ...
  const studentResults=periodTests.map(t=>resultFor(t,selected)).filter(Boolean);
  const studentAttended=studentResults.filter(r=>r.present!==false&&!r.na&&r.marks!=null).length;
  ...
  const avg=a=>a.length?Math.round(a.reduce((x,y)=>x+y,0)/a.length*10)/10:null;
  const overall=avg(currentScores), attendance=periodTests.length?Math.round(studentAttended/periodTests.length*1000)/10:null;
  ...
  return {...base,month:monthlyMonthLabel(latestKey),summary:overall==null?'No scored assessments are available for this student in the selected period.':`${selected.name} has an overall average of ${overall}%. ${best?`${best[0]} is the strongest subject.`:''} ${needs?`${needs[0]} needs additional practice.`:'No separate subject needs revision yet.'}`,kpis:[['Overall Average',overall==null?'—':`${overall}%`,''],['Tests Conducted',String(periodTests.length),''],['Improvement',improvement==null?'—':`${improvement>=0?'+':''}${improvement}%`,''],['Exam Attendance',attendance==null?'—':`${attendance}%`,`${studentAttended} of ${periodTests.length} exams attended`],['Focus Subject',needs?monthlyShortSubject(needs[0]):'No single focus','']],subjects:subjectRows,trend:{labels:subjectRows.map(s=>monthlyShortSubject(s[0])),student:subjectRows.map(s=>parseFloat(s[2])||0),klass:subjectRows.map(s=>parseFloat(s[3])||0),topper:subjectRows.map(s=>parseFloat(s[4])||0)},highlights:[],insight:[best?`${best[0]} is the strongest subject this month.`:'No strength identified yet.',needs?`${needs[0]} needs more practice.`:'No subject is currently below the revision threshold.',improvement!=null?`Performance changed ${improvement>=0?'up':'down'} ${Math.abs(improvement)} points from the previous month.`:'Keep attempting regular chapter tests.'],plan:[needs?`Revise ${needs[0]} concepts`:'Review the next available chapter','Practise weekly worksheets','Discuss progress with the teacher','Attempt more practice tests']};
}
...
async function monthlyReport(){
  ...
  // monthlyBuildData may snap MONTHLY_FILTER.month to the selected student's
  // latest month that actually has test data. Rebuild the selector markup so the
  // Month dropdown matches the report that is rendered below it.
  const selectorHtml=selected?monthlySelectorMarkup(live,MONTHLY_SAMPLE.student).html:selector.html;
  const d=MONTHLY_SAMPLE;
  document.getElementById('app').innerHTML=`<main class="monthly-report" id="monthlyReport">
    <div class="monthly-actions">${selectorHtml}<div class="monthly-report-actions"><button class="monthly-whatsapp-button" onclick="monthlyShareWhatsApp()">💬 Share WhatsApp</button><button class="primary" onclick="printMonthlyReport()">🖨 Print / Save as PDF</button></div></div>
    <header class="monthly-cover"><div class="monthly-brand"><img src="assets/images/aveti-logo.png" alt="Aveti Learning Tuition Center"><div class="monthly-brand-copy"><div class="monthly-eyebrow">AVETI LEARNING TUITION CENTER</div><div class="monthly-brand-contact">${escMonthly(CONFIG.CENTRE?.address||'M74, Baramunda Housing Board Colony')} · Ph ${escMonthly(CONFIG.CENTRE?.phone||'98619 27954')}</div><h1>MONTHLY PROGRESS REPORT</h1></div></div></header>
