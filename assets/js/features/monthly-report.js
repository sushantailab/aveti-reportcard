...
async function monthlyBuildData(selected,students){
  ...
  return {...base,month:monthlyMonthLabel(latestKey),summary:overall==null?'No scored assessments are available for this student in the selected period.':`${selected.name} has an overall average of ${overall}% across ${currentScores.length} attended assessment${currentScores.length===1?'':'s'}. ${best?`${best[0]} is the strongest subject.`:''} ${needs?`${needs[0]} needs additional practice.`:'No separate subject needs revision yet.'}`,kpis:[['Overall Average',overall==null?'—':`${overall}%`,''],['Tests Conducted',String(periodTests.length),''],['Improvement',improvement==null?'—':`${improvement>=0?'+':''}${improvement}%`,''],['Exam Attendance',attendance==null?'—':`${attendance}%`,''],['Focus Subject',needs?needs[0]:'No single focus',''],['Discipline',discipline,'']],subjects:subjectRows,trend:{labels:subjectRows.map(s=>monthlyShortSubject(s[0])),student:subjectRows.map(s=>parseFloat(s[2])||0),klass:subjectRows.map(s=>parseFloat(s[3])||0),topper:subjectRows.map(s=>parseFloat(s[4])||0)},highlights:[best&&['Best performance',best[0],'Highest score this month',best[2],'best'],needs&&['Needs revision',needs[0],'Focus subject',needs[2],'needs'],improvement!=null&&[improvement>=0?'Most improved':'Declined this month',selected.name,'Compared with previous month',`${improvement>=0?'+':''}${improvement}%`,improvement>=0?'improved':'needs']].filter(Boolean),insight:[best?`${best[0]} is the strongest subject this month.`:'No strength identified yet.',needs?`${needs[0]} needs more practice.`:'No subject is currently below the revision threshold.',improvement!=null?`Performance changed ${improvement>=0?'up':'down'} ${Math.abs(improvement)} points from the previous month.`:'Keep attempting regular chapter tests.'],plan:[needs?`Revise ${needs[0]} concepts`:'Review the next available chapter','Practise weekly worksheets','Discuss progress with the teacher','Attempt more practice tests']};
}
...
async function monthlyReport(){
  ...
  document.getElementById('app').innerHTML=`<main class="monthly-report" id="monthlyReport">
    ...
    <section class="monthly-page-two"><h2>Performance scorecard</h2><div class="monthly-overall">${[['Overall progress',d.kpis[0][1],d.kpis[0][1]==='—'?'Not available':'Based on attended assessments'],['Improvement',d.kpis[2][1],d.kpis[2][1]==='—'?'Not available':(String(d.kpis[2][1]).startsWith('-')?'Declined from previous month':'Improved from previous month')],['Consistency',d.kpis[1][1]==='0'?'Not available':'Very good',d.kpis[1][1]==='0'?'No tests conducted':'Regular participation'],['Discipline',d.kpis[5][1], 'Fixed centre rating']].map((x,i)=>`<article class="${i===2?'best':'good'}"><span class="summary-icon">${monthlyIcon(['chart','trend','check','star'][i])}</span><small>${x[0]}</small><strong>${escMonthly(x[1])}</strong><em>${x[2]}</em></article>`).join('')}</div><h2>Achievement badges</h2><div class="monthly-badges">${[['Science star','trophy'],['Most improved student','trend'],['Perfect exam attendance','target'],['Homework champion','book'],['Excellent discipline','star']].map((x,i)=>`<div class="badge-${i}"><span>${monthlyIcon(x[1])}</span><b>${x[0]}</b></div>`).join('')}</div><section class="monthly-parent-guide"><h2>Parent action guide</h2><ul><li>Encourage daily reading.</li><li>Ask one question after every class.</li><li>Help your child attempt more practice tests.</li></ul></section><div class="monthly-signatures"><div><b>Academic head</b><strong>AVETI Learning Tuition Center</strong><hr><span>Generated: ${new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(new Date())}</span></div></div></section>
  </main>`;
}
