...
async function monthlyReport(){
  ...
  document.getElementById('app').innerHTML=`<main class="monthly-report" id="monthlyReport">
    ...
    <section class="monthly-page-two"><h2>Executive performance summary</h2><div class="monthly-overall">${[['Overall progress','Excellent','Good overall performance'],['Improvement','Good','Positive month-on-month movement'],['Consistency','Very good','Regular participation'],['Discipline','Good','Healthy learning habits']].map((x,i)=>`<article class="${i===2?'best':'good'}"><span class="summary-icon">${monthlyIcon(['chart','trend','check','star'][i])}</span><small>${x[0]}</small><strong>${x[1]}</strong><em>${x[2]}</em></article>`).join('')}</div><h2>Achievement badges</h2><div class="monthly-badges">${['Science star','Most improved student','Perfect exam attendance','Homework champion','Excellent discipline'].map((x,i)=>`<div><span>${monthlyIcon('badge')}</span><b>${x}</b></div>`).join('')}</div><section class="monthly-parent-guide"><h2>Parent action guide</h2><ul><li>Encourage daily reading.</li><li>Ask one question after every class.</li><li>Help your child attempt more practice tests.</li></ul></section><div class="monthly-signatures"><div><b>Academic head</b><strong>AVETI Learning Tuition Center</strong><hr><span>Generated: ${new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(new Date())}</span></div></div></section>
  </main>`;
}
