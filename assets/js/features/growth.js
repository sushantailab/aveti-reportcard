  ...
  if(GR.mode==='ind'){
    ...
    show(`
      ...
      <div class="growth-dashboard growth-individual-dashboard">
        ...
        <div class="growth-head"><img class="growth-print-logo" alt="Aveti Learning Tuition Center logo" src="assets/images/aveti-logo.png"><div><div class="eyebrow">Aveti Learning Tuition Center</div><div class="growth-print-contact">${CONFIG.CENTRE.address}${CONFIG.CENTRE.phone?` · Ph ${CONFIG.CENTRE.phone}`:''}</div><h1>Class ${GR.cls} — Individual student progress report</h1><div class="tiny muted">${GR.section==='All'?'All sections':`Section ${GR.section}`} · ${currentSession()}</div></div><div class="growth-controls"><button class="growth-print-button" onclick="printGrowthReport()">🖨 Print / Save as PDF</button><button class="growth-whatsapp-button" onclick="shareGrowthSummary()">💬 WhatsApp summary</button><div class="seg"><button onclick="setGrowthMode('class')">Class trend</button><button class="on" onclick="setGrowthMode('ind')">Individual</button></div><select style="width:auto" onchange="setGrowthStudent(this.value)">${growthStudentOptions(classStudents)}</select></div></div>
        ${growthReportMeta()}
...
window.printGrowthReport = ()=>{
  const student = GR.student ? document.querySelector('.growth-student-band b')?.textContent : '';
  const parts = [`Class-${GR.cls}`,GR.subject,GR.mode==='ind'?'Individual-Student-Progress':'Chapterwise-Growth','Report'];
  if(student) parts.splice(2,0,student);
  printReportWithFilename(reportPdfFilename(...parts));
};
